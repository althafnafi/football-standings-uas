...
void showCombinedRankingsMenu(Tour* ptr, Node** root, int n)
{
    ...
    if (n == -1)
    {
        clearAndPrintHeader("Combined Team Rankings");
    ...
    do {
        ...
        count = 0;
        exit_to_menu = 0;
        /* show combined team rankings */
        clearAndPrintHeader("Combined Team Rankings");
        printf("Combined Rankings of Teams based on the \"Team Index Scoring System\"\n");
        printCombinedRankings(*root);
        printf("\n");
        printf("<< Menu Options >>\n");
        printf("1. Search for a team by name\n");
        printf("2. Search for a team by rank\n");
        printf("3. Exclude a team from the rankings (permanently)\n");
        printf("4. Read more about the Team Index Scoring System\n");
        ...
        {
            case '1':
                /* search for a team based on name */
                clearAndPrintHeader("Search by name");
                printf("\nEnter the team's name:\n>> ");
                fflush(stdin); scanf("%[^\n]s", t_name); fflush(stdin);
                searchNameBst(*root, &found, t_name);
                if (found != 1)
                    printf("Team name not found! Input has to match exactly!\n");
                anyKey();
                break;
            case '2':
                /* search for a team based on rank */
                clearAndPrintHeader("Search by rank");
                printf("\nEnter the team's rank:\n>> ");
                fflush(stdin); scanf("%d", &rank); fflush(stdin);
                searchRankBst(*root, &found, &count, rank);
                if (found != 1)
                    printf("Team rank is invalid!\n");
                anyKey();
                break;
            case '3':
                clearAndPrintHeader("Delete a team from the combined rank permanently");
                printf("\n");
                printCombinedRankings(*root);
                printf("Enter the team's Team Index Score:\n>> ");
                fflush(stdin); scanf("%f", &t_tis); fflush(stdin);
                deleteNode(*root, t_tis);
                printf("The team with that Team Index Score has been excluded!\n");
                anyKey();
                break;
            case '4':
                helpTISS();
    ...
    } while (exit_to_menu == 0);
    return;
}

void helpTISS()
{
    clearAndPrintHeader("About the Team Index Scoring System");
    printf("\nThe team index scoring system is our way of evaluating teams based on\n");
    printf("their performance in past tournaments.\n");
    printf("We calculate the team index score by using the following formula:\n");
    printf("Games Played * 5 + (Wins/Losses)* 50 + Draws * 2.5 + Goal Diff. * 15\n\n");
    printf("In this menu user can opt to search for teams based on their name or rank,\n");
    printf("and even delete/exclude a team for this ranking list\n\n");
    anyKey();
    return;
}

/* ---------------------------- bst functions ------------------------------- */
// print all the nodes in the bst inorder w/ right bias
void printCombinedRankings(Node* root)
{
    if (root != NULL) 
    {
        printCombinedRankings(root->right);
        printf(" %.1f pts - ", root->tis);
        printf("%s\n", root->team_name);
        printCombinedRankings(root->left);
    }
}
...
// A dfs algorithm to scan all the nodes in the bst to find the team name
void searchNameBst(Node* root, int* found, char* t_name)
{
    if (root != NULL)
    {
        if (strcmp(root->team_name, t_name) == 0)
        {
            printf("\nFound!\n");
            printf(" %.1f pts - %s\n", root->tis, root->team_name);
            *found = 1;
        }

        searchNameBst(root->left, found, t_name);
        searchNameBst(root->right, found, t_name);
    }
}
...
void searchRankBst(Node* root, int* found, int* count, int rank)
{
    if (root != NULL)
    {
        searchRankBst(root->right, found, count, rank);
        ...
        *count += 1;
        ...
        searchRankBst(root->left, found, count, rank);
    }
}
...
void insertNode(Node** node, float ti_score, char team_name[])
{
    ...
    if (*node == NULL)
    {
        *node = newNode(ti_score, team_name);
    }

    else 
    {
        // if node is not null traverse through the bst
        if (ti_score < (*node)->tis)
            insertNode(&(*node)->left, ti_score, team_name);
        else
            insertNode(&(*node)->right, ti_score, team_name);
    }
}
...
struct bstNode* minValueNode(Node* node)
{
    ...
    // find the minimum value or the leftmost node
...
struct bstNode* deleteNode(Node* root, float key)
{
    ...
    // if key is smaller than node, go left
    if (key < root->tis)
        root->left = deleteNode(root->left, key);

    // if key is larger than node, go right
    else if (key > root->tis)
        root->right = deleteNode(root->right, key);

    // if key is the same as node, do the deletion
    else
    {
        ...
        else
        {
            ...
            root->tis = temp->tis;
            root->right = deleteNode(root->right, temp->tis);
        }
    }
    return root;
}
...
void mainMenu(Tour* ptr, Node** root, int n) 
{
    ...
    while (exit_prog == 0)
    {
        ...
        puts(" 3. Show combined rankings for all teams");
        puts(" 4. Help");
        puts(" 5. About");
        ...
        switch(choice) 
        {
            case '1':
                ...
                n++;
                calcStandings(ptr, root, n);
                getch();
                break;
            case '2':
                // menu 2 calls the function to see the previous standings
                showPrevStandings(ptr, root, n);
                break;
            case '3':
                // menu 3 calls function to show combined rankings
                showCombinedRankingsMenu(ptr, root, n);
                break;
            case '4':
                // menu 4 calls the function to show the help
                helpMainMenu(ptr, n);
                break;
            case '5':
                // menu 5 calls the function to show the about page/menu
                aboutMenu(ptr, n);
                break;
            case '6':
                // menu 6 calls the function to exit the program
                exit_prog = 1;
                exitMenu(&exit_prog);
                break;
            default:
                // if user enters other than 1...6
                break;
        }
    }
    exit(0);
...
void calcStandings(Tour* ptr, Node** node, int n) 
{
    ...
    for (i = 0; i < ptr[n].num_teams; i++)
    {
        ...
        /* ----------------------------------------------*/
        /* For combined rankings with binary search tree */
        /* ----------------------------------------------*/
        /* find ti_score */
        float ti_score =
        ptr[n].teams[i].games_played * 5 + (ptr[n].teams[i].wins - ptr[n].teams[i].losses) * 50
        + ptr[n].teams[i].draws * 2.5 + ptr[n].teams[i].goal_difference * 15;
        /* initialize a new node and insert into the binary tree */
        insertNode(node, ti_score, ptr[n].teams[i].name);
    }

    standingsMenu(ptr, node, n, -1);
}
...
void standingsMenu(Tour* ptr, Node** root, int n, int tour_index)
{
    ...
    switch(getch())
    {
        case '1': // search by team name
            ...
            if (target_index == -1)
            {
                standingsMenu(ptr, root, n, temp_index);
                break;
            }
...
{   
    ...
    else if (match_num >= tourney.num_matches) // goes above max
    {
        match_num = tourney.num_matches - 1;
    }
    ...
    {
        printf("\t<<   %d/%d   >>\n\n", match_num+1, tourney.num_matches);
    }
    ...
    printf(
        ">> %s VS %s <<\n",
        tourney.matches[match_num].teamA_name,
        tourney.matches[match_num].teamB_name
    );
    printf(" ");

    // print as long as team A's name (for formatting purposes)
    for (i = 0; i < strlen(tourney.matches[match_num].teamA_name); i++)
    {
        printf(" ");
    }
    printf(
        "%d  -  %d", 
        tourney.matches[match_num].teamA_score,
        tourney.matches[match_num].teamB_score
    );

    // print goal details
    printf("\n\nGoal Details:\n");
    for (i = 0; i < tourney.matches[match_num].total_goals; i++)
    {   
        if (tourney.matches[match_num].input_status != 4)
        {
            printf(" - %s ", tourney.matches[match_num].scorer[i]);
            if (tourney.matches[match_num].goal_time[i] != -1)
            {
                printf("(%d')", tourney.matches[match_num].goal_time[i]);
            }
            printf("\n");
        } 
    ...
    if (strcmp(mode, "scroll") == 0)
    {
        ...
        switch(inputArrowKey()) // ask for arrow key input
        {
            case 1: case 2: // go to the left page
                showMatchDetails(tourney, match_num-1, mode);
                break;
...
void showPrevStandings(Tour* ptr, Node **root, int n)
{
    ...
    standingsMenu(ptr, root, n, input-1);
}
...
void exitMenu(int *exit_prog)
{
    ...
    // if "n" then go back to main menu
    else
    {
        *exit_prog = 0;
        return;
